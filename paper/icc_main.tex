...
\end{equation}
where $q(t)$ is the link-score term and $w_q, w_f, w_e$ are tunable utility weights. The optimizer selects the window with maximum $U(t)$ within a 6-hour planning horizon, balancing link quality, Doppler rate, and energy expenditure. Algorithm~1 summarizes the end-to-end terminal-side procedure.
...
This section supports the transmitter-configuration and signal-observability path of Fig.~\ref{fig:architecture}; receiver-in-the-loop validation is future work.
...
\begin{figure}[t]
    ...
    \caption{Conducted LR1121$\rightarrow$USRP B210 IQ capture at 868~MHz: TX-ON vs TX-OFF max-hold spectrum from the raw IQ ($\Delta=9.82$~dB; DC/LO guard shaded). Reported at the conducted IQ-signal layer; receiver-in-the-loop validation is future work.}
    \label{fig:sdr}
\end{figure}
