...
\begin{figure}[t]
    ...
    \node[box] (tle) {TLE /\\SGP4--SDP4};
    \node[box, right=of tle, fill=blue!10] (pgrl) {PGRL predictor\\frozen mean +\\calibrated log-var head};
    \node[box, right=of pgrl] (out) {PGRL output\\mean state + per-axis $\sigma$};
    \draw[ar] (tle)--(pgrl);
    \draw[ar] (pgrl)--(out);
    % Band 2: risk-aware uncertainty-aware control
    \node[ctrl, below=8mm of pgrl] (guard) {risk-aware guard\\$g_{\text{base}}+\alpha\sigma_r$};
    \node[ctrl, right=of guard] (txt) {TX timing\\selection};
    \node[ctrl, right=of txt] (dop) {Doppler\\pre-comp};
    \draw[ar] (out.south) |- (dop.east);
    \draw[ar] (dop)--(txt);
    \draw[ar] (txt)--(guard);
    % Band 3: conducted IQ evidence (no decode / no PER)
    \node[hw, below=8mm of guard] (lr) {LR1121 TX};
    \node[hw, right=of lr] (usrp) {USRP B210\\IQ capture};
    \node[align=left, right=5mm of usrp, font=\scriptsize\itshape] (plan) {conducted IQ\\capture only;\\no decode / no PER};
