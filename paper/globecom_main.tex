...
Training uses the GRPO (Group Relative Policy Optimization) variant of PPO to balance NLL minimization with physics residual regularization; the Bayesian output head maintains calibrated uncertainty. On the evaluation dataset (multiple satellites, 30--120~min prediction horizons), PGRL achieves 16~ms pass-timing RMSE versus 4.2~s for SGP4 alone, and residual Doppler $<300$~Hz versus $>5$~kHz for SGP4 alone, as shown in Table~\ref{tab:main}. The NLL decreases from 3.01 (SGP4) to 0.17 (PGRL), confirming that the uncertainty estimates are well-calibrated and that the predictor captures the non-deterministic components of orbital dynamics that SGP4 misses.
